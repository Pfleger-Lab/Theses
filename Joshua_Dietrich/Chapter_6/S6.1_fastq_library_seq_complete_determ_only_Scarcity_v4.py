...
filepath = os.getcwd()
filename = sys.argv[1] #'2023_0331_merged_pass.fastq'
...
for idx,curr_read_rec in enumerate(reads):
    ...
    # print('Read',curr_read_rec.id, curr_read_length)
    if round(idx/num_of_reads*100) > curr_progress:
        curr_progress = round(idx/num_of_reads*100)
    ...
    for idx,(result,out_file) in enumerate(zip(results,out_files)):
        with open(result,'r') as result_handle:
            for record in NCBIXML.parse(result_handle):
                out_file.append("Query:\t"+str(record.query.split()[0]))
